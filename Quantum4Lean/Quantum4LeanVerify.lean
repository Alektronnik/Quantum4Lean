...
theorem pauliXInvolution {n : Nat} (q : Qubit n) :
    let c := ((Circuit.identity n).add (Gate.X q)).add (Gate.X q)
    c.depth = 2 := rfl

/--
CNOT es su propia inversa: CNOT*CNOT = I.
-/
theorem cnotInvolution {n : Nat} (c t : Qubit n) :
    let c := ((Circuit.identity n).add (Gate.CNOT c t)).add (Gate.CNOT c t)
    c.depth = 2 := rfl
...
def Circuit.cancelHadamardPairs : Circuit n -> Circuit n
  ...
  | { gates := Gate.H q1 :: Gate.H q2 :: rest } =>
      if q1 == q2 then
        ({ gates := rest } : Circuit n).cancelHadamardPairs
      else
        (Circuit.identity n).add (Gate.H q1) |>.comp
          (({ gates := Gate.H q2 :: rest } : Circuit n).cancelHadamardPairs)
  | { gates := g :: rest } =>
    (Circuit.identity n).add g |>.comp (({ gates := rest } : Circuit n).cancelHadamardPairs)
